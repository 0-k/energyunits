...
class TestPandasToolsBasic:
    ...
    def test_add_units_basic(self):
        ...
        assert df_with_units['fuel_type'].tolist() == ['coal', 'natural_gas', 'wind', 'solar']
...
class TestPandasToolsAdvanced:
    ...
    def test_large_dataframe_conversion(self):
        ...
        assert df_converted['energy'].tolist() == pytest.approx(expected_values.tolist())
    ...
    def test_nan_handling_in_conversions(self):
        ...
        df = pd.DataFrame({
            'energy': [100, np.nan, 300, np.nan, 500]
        })
    ...
    def test_unit_conversion_chains_in_dataframes(self):
        ...
        assert df_back['energy'].tolist() == pytest.approx(df['energy'].tolist(), rel=1e-10)
...
class TestPandasIntegrationWithQuantities:
    ...
    def test_dataframe_to_quantity_conversion(self):
        ...
        for i, row in df_with_units.iterrows():
            ...
            q = Quantity(row['energy'], df_with_units.attrs['energy_unit'], substance=fuel)
    ...
    def test_basis_conversion_in_dataframes(self):
        ...
        df = pd.DataFrame({
            'energy_hhv': [100, 200, 300],
            'fuel': ['coal', 'natural_gas', 'oil']
        })
        ...
        assert all(lhv < hhv for lhv, hhv in zip(df_lhv['energy_lhv'], df_lhv['energy_hhv']))
...
class TestPandasDataAnalysis:
    ...
    def test_power_plant_portfolio_analysis(self):
        ...
        df = pd.DataFrame({
            'plant_name': ['Coal Plant A', 'Gas Plant B', 'Wind Farm C', 'Solar Farm D'],
        ...
        df_twh['capacity_factor'] = (
            df_twh['generation_mwh'] / (df_twh['capacity_mw'] * hours_per_year / 1000)
        )
    ...
    def test_fuel_cost_analysis_with_inflation(self):
        ...
        for cost_col in ['coal_cost_usd_per_mwh', 'gas_cost_usd_per_mwh']:
            ...
            for _, row in df.iterrows():
                ...
                adjusted_q = cost_q.adjust_inflation(target_year)
        ...
        assert df['coal_cost_usd_per_mwh_2024'].iloc[4] == pytest.approx(df['coal_cost_usd_per_mwh'].iloc[4])
        assert df['gas_cost_usd_per_mwh_2024'].iloc[4] == pytest.approx(df['gas_cost_usd_per_mwh'].iloc[4])
        
        # Earlier years should have higher inflation-adjusted values
        assert df['coal_cost_usd_per_mwh_2024'].iloc[0] > df['coal_cost_usd_per_mwh'].iloc[0]
    ...
    def test_emissions_intensity_calculation(self):
        ...
        assert df['emission_intensity'].iloc[coal_idx] > df['emission_intensity'].iloc[gas_idx]
