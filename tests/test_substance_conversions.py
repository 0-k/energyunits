...
from energyunits.exceptions import UnknownSubstanceError
...
class TestSubstanceConversions:
    ...
    def test_all_combustion_products_different_fuels(self):
        ...
        test_cases = [
            ("lignite", {"carbon_content": 0.65, "hydrogen_content": 0.04, "ash_content": 0.15}),
            ("anthracite", {"carbon_content": 0.85, "hydrogen_content": 0.04, "ash_content": 0.05}),
            ("diesel", {"carbon_content": 0.86, "hydrogen_content": 0.14, "ash_content": 0.0}),
            ("wood_pellets", {"carbon_content": 0.50, "hydrogen_content": 0.06, "ash_content": 0.01}),
        ]
        ...
        for fuel_name, composition in test_cases:
            ...
            assert co2.value == pytest.approx(expected_co2), f"CO2 calculation failed for {fuel_name}"
            
            # Test H2O
            h2o = fuel.to(substance="H2O")
            expected_h2o = composition["hydrogen_content"] * (18/2)
            assert h2o.value == pytest.approx(expected_h2o), f"H2O calculation failed for {fuel_name}"
            
            # Test ash
            ash = fuel.to(substance="ash")
            expected_ash = composition["ash_content"]
            assert ash.value == pytest.approx(expected_ash), f"Ash calculation failed for {fuel_name}"
    ...
    def test_cross_substance_consistency(self):
        ...
        for fuel in fuels_to_test:
            ...
            assert stored_intensity >= 0, f"Stored intensity should be non-negative for {fuel}"
            ...
            if fuel != "wood_pellets":  # Biomass might be considered carbon neutral
                assert 100 <= co2_kg_per_mwh <= 500, f"CO2 intensity out of range for {fuel}: {co2_kg_per_mwh}"
...
class TestSubstanceRegistryDirectly:
    ...
    def test_registry_access_all_substances(self):
        ...
        expected_substances = [
            ...
            "natural_gas", "lng", "methane",
            "crude_oil", "oil", "fuel_oil", "diesel", "gasoline",
            "wood_pellets", "wood_chips",
